...
export const useInboxFilter = ({ filters = inboxFilters, defaultFilterState = {} }) => {
  ...
  const getFolderLabel = () => {
    const values = filterState?.folder || [];
    const count = values?.length;

    const options = filters?.find((item) => item.name === 'folder')?.items;
    const labels = values?.map((value) => options?.find((option) => option.value === value)?.title);

    const statusLabel = count === 1 ? labels?.[0] : count > 1 ? `${count} mapper` : undefined;
    return [statusLabel].filter(Boolean).join(' + ');
  };

  const getFilterLabel = (name: string) => {
    switch (name) {
      case 'folder':
        return getFolderLabel() || 'Mappe';
      case 'status':
