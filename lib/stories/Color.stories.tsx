...
const ColorSwatch = ({ name, data, children }: ColorTokenProps) => {
  ...
  return <div style={style}>{children || data?.$value}</div>;
};

const ColorGroup = ({ colorName, tokens }: { colorName: string; tokens: TokenGroup }) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      <ColorSwatch name={colorName}>{colorName}</ColorSwatch>
      <div style={{ display: 'flex', flexDirection: 'row' }}>
        {Object.entries(tokens).map(([tokenName, tokenData], index: number) => (
          <ColorSwatch key={tokenName} name={`${colorName}-${tokenName}`} data={tokenData}>
            {index + 1}
          </ColorSwatch>
        ))}
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      {Object.entries(theme.color).map(([colorName, tokens]) => (
        <ColorGroup key={colorName} colorName={colorName} tokens={tokens} />
      ))}
    </div>
  );
};
