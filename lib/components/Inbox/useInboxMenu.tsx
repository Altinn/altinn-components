...
export const useInboxLayout = ({
  ...
  const menuItems = items.map((item) => {
    const storyBookId =
      storybookPages?.[item.id as keyof typeof storybookPages];
    const href =
      storyBookId &&
      [baseHref, storyBookId].join("") + "&accountId=" + accountId;
  ...
  const mobileMenuChildren = menuItems
    ?.slice(1)
    ?.filter((item) => item.groupId !== "shortcuts"); // all except top node and shortcuts
