...
A dialog must include a `title` and a `summary`, a `sender` and `recipient`, as well as a timestamp representing when the dialog was created or updated.
...
When a dialog is opened, main content is highlighted with a border and introduced with a timestamp, representing the time the dialog was last updated. The `summary` might include a list of `attachments` and/or one or more call to `actions`.
