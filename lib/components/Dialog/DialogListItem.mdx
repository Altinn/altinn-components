...
New dialogs are unseen until they have been opened by the current user. In list views, new dialogs are indicated using three clues; it has a coloured border, a stronger title weight and an optional `badge`, emphasising that the dialog is "New". When a dialog is opened the dialog is marked as seen by the end user, the border and title weight is neutralised, and the badge is removed.
...
## Drafts and sent folder

Processes that have been initiated by the end user, for example by filling out a form, should not appear in the main inbox before they receive a response. These types of dialogs should be mapped to the "Drafts" og "Sent" folders respectively.
