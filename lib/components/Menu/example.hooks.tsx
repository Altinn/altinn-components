...
export const useRandomMenuItems = (limit = 10) => {
  ...
  const randomCompanies = generateRandomCompanies(limit);
  ...
  const items = [
    ...
    const searchWords = [item.name, item.orgNumber, item.birthdate] as string[];
  ...
  return { items, groups: groups as MenuProps['groups'] };
};
